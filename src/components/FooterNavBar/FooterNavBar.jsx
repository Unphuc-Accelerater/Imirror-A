...
export const FooterNavBar = () => {
  ...
  const renderIcon = (name, active) => {
    const color = active ? '#2d275b' : '#b4b0bb';
      ...
      >
        ...
        <motion.span 
          className={`text-xs font-medium mt-1 transition-colors duration-300 ${
            active ? 'text-[#2d275b]' : 'text-[#b4b0bb]'
          }`}
  ...
  return (
    ...
    >
      <div className="bg-[#f6edf6] backdrop-blur-xl shadow-2xl rounded-t-3xl">
        <div className="flex items-center justify-around px-4 py-2 max-w-md mx-auto relative h-16">
          ...
          >
            <motion.button
              className="w-18 h-18 bg-[#2d275b] rounded-full shadow-xl flex items-center justify-center -mt-6 border-4 border-[#f6edf6] relative overflow-hidden"
              onClick={() => navigate('/request-feedback')}
              whileHover={{ 
                scale: 1.05,
                boxShadow: "0 20px 40px rgba(45, 39, 91, 0.4)"
              }}
              whileTap={{ scale: 0.95 }}
            >
        ...
        <div className="h-safe-bottom bg-[#f6edf6]" />
      </div>
    </motion.div>
  );
};